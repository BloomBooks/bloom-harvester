	...
	class BookAnalyzer : IBookAnalyzer
	{
		...
		public bool IsEpubSuitable(List<LogEntry> harvestLogEntries)
		{
			...
			foreach (var div in GetNumberedPages().ToList())
			{
				...
				{
					harvestLogEntries.Add(new LogEntry(LogLevel.Warn, LogType.ArtifactSuitability, "No ePUB because some page(s) had multiple images"));
					return false;
				}
				...
				{
					harvestLogEntries.Add(new LogEntry(LogLevel.Warn, LogType.ArtifactSuitability, "No ePUB because some page(s) had multiple text boxes"));
					return false;
				}
				var videos = div.SafeSelectNodes("following-sibling::div[contains(@class,'marginBox')]//video");
				if (videos.Count > 1)
				{
					harvestLogEntries.Add(new LogEntry(LogLevel.Warn, LogType.ArtifactSuitability, "No ePUB because some page(s) had multiple videos"));
					return false;
				}
				++goodPages;
			}
			if (goodPages == 0)
				harvestLogEntries.Add(new LogEntry(LogLevel.Warn, LogType.ArtifactSuitability, "No ePUB because there were no content pages"));
			return goodPages > 0;
		}
